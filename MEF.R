...
# write.csv(completedData, "BCI_trait_data_imp.csv") #write new .csv file with imputed trait data
...
run.maxent.par(data=vegtab,traitdata="BCI_trait_data_imp.csv", perm=3, clustertype="FORK")
# save(total.output, file = "output_maxent_alltraitsmice.RData") #save the results from MEF
...
##########################################################################################
#### Plot heatmap showing the ratio between pure metacommunity and pure trait effects ####
##########################################################################################

colnames(merge.results)
metatrait_ratio = merge.results[,11:12]
rownames(metatrait_ratio) = rownames(vegtab)
x = (metatrait_ratio$pure.metacommunity.effect)/(10*(metatrait_ratio$pure.trait.effect+1))
metatrait_ratio = data.frame(cbind(data_env$UTM.EW,data_env$UTM.NS ,x))
colnames(metatrait_ratio) = c("UTM.EW","UTM.NS","x")

ggplot(metatrait_ratio) + 
geom_tile(aes(x=UTM.EW, y = UTM.NS, fill=x*1000)) + 
  coord_fixed(ratio = 1) +
...
#############################################################
#### Boxplots concerning ratio mc.effect/trait effection ####
#############################################################

ratiocalc = (results$pure.metacommunity.effect)/(10*(results$pure.trait.effect+1))
ratio = data.frame(ratiocalc)
ratio$Habitat = results$Habitat
test.m <- melt(ratio)

myplot = ggplot(test.m, aes(x = reorder(Habitat, value, FUN=median), y = value, fill = Habitat)) +
...
tofill = matrix(nrow=14, ncol = length(unique(forest.types))+1)
rownames(tofill) = colnames(data.output)
colnames(tofill) = c(as.character(unique(forest.types)),"Combined")
...
tofill[,i] = data.output3
}

tofill[,6] = apply(data.output,2,mean)
rownames(tofill) = c("Rkl(u)","Rkl(u,t)","Rkl(m)","Rkl(m,t)","ΛR2KL(t|φ)","ΛR2KL(m|φ)","ΛR2KL(t|m)","ΛR2KL(m|t)","ΛR2KL(m+t)","1-ΛR2KL(m+t)","Pure trait effect","Pure metacommunity effect","Joint effect","Unexplained effects")
...
#Load already trimmed data from Server
...
##############################################################
### Plot distance category versus pure metacommunity effect ###
##############################################################

##Create dataframe to check differences in minimum versus maximum metacommunity effect
names = unique(effects_distance$Plotcode)
for (i in 1:length(names)){
selection = effects_distance[effects_distance$Plotcode == names[i],]
minimum = min(selection$pure.metacommunity.effect)
maximum = max(selection$pure.metacommunity.effect)
difference = maximum-minimum
if(i == 1){
agg_selection <<- data.frame(names[i],minimum,maximum, difference)} else {
to_add = data.frame(names[i], minimum, maximum,difference)
agg_selection = rbind(agg_selection,to_add)}}
agg_selection = agg_selection[order(agg_selection$difference),]
agg_selection

#Mean decrease in metacommunity effect
foresttypes = as.character(unique(data_env$Habitat))
agg_selection$Forest = "na"
fill = matrix(nrow=6, ncol=2)

for(i in 1:length(foresttypes)){
selectforesttype = data_env[data_env$Habitat==foresttypes[i],]
agg_selection[agg_selection$names.i. %in% as.numeric(rownames(selectforesttype)),]$Habitat = foresttypes[i]
select = agg_selection[agg_selection $Forest == foresttypes[i],]
fill[i,2] = mean(select$difference)}

fill[,1] = c(foresttypes, "ALL")
fill[6,2] = mean(agg_selection$difference)
fill
